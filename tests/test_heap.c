...
void test_heap_create(void) {
    ...
    int array_size = 5;
    Heap h = heap_create(data_array, data_of_interest, array_size);
    TEST_CHECK(h != NULL);
    TEST_CHECK(h->size == 5);
    TEST_CHECK(h->capacity >= 5);
    TEST_CHECK(h->data_of_interest == 1);
    TEST_CHECK(heap_pop(h) == 8);
    // TEST_CHECK(h->array[1].index == 5);
    // TEST_CHECK(h->array[2].index == 3);
...
void test_heap_insert(void) {
    ...
    TEST_CHECK(h->array[1].index == 6);
    TEST_CHECK(h->array[2].index == 3);
    TEST_CHECK(h->array[3].index == 2);
    TEST_CHECK(h->array[4].index == 1);
    TEST_CHECK(h->array[5].index == 5);
    heap_destroy(h);
}
...
void test_heap_update(void) {
    ...
    int array_size = 5;
    Heap h = heap_create(data_array, data_of_interest, array_size);
    bool ret = heap_update(h, 8, 2);
    TEST_CHECK(ret == true);
    TEST_CHECK(h->array[0].index == 5);
    TEST_CHECK(h->array[1].index == 3);
    TEST_CHECK(h->array[2].index == 2);
    TEST_CHECK(h->array[3].index == 6);
    TEST_CHECK(h->array[4].index == 1);
    heap_destroy(h);
}
